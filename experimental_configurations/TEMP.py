...
def visualize_results(*, mesh, uh, cells_1, cells_2, save_path=None, experiment_id=None):
...
def run_experiment(experiment_id, mesh, save_path, kappa1, kappa2, ratio,
    ...
    f = Constant(mesh, default_scalar_type(10))  # Heat source
    ...
    with open(f"{save_path}/simulation_metadata_{experiment_id}.json", "w") as file:
        metadata = {
            ...
            "materials": {"kappa1": kappa1, "kappa2": kappa2},
    ...
    return {
        ...
        "Uh": uh.x.array,             # Include `uh` for visualization
        # "V": V,               # Include `V` for visualization
        "cells_1": cells_1,   # Include `cells_1` for visualization
        "cells_2": cells_2,   # Include `cells_2` for visualization
...
def main():
    ...
    save_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), "results")
    csv_file = os.path.join(os.path.abspath(os.path.dirname(__file__)), "mpi_timing_results.csv")
    num_experiments = 32  # Total number of experiments
    kappa1 = 400
    kappa2 = 2.4
    ...
    experiments_per_rank = num_experiments // size
    ...
    for i in my_experiments:
        result = run_experiment(
            i + 1, mesh, save_path, ratio=((i + 1) / num_experiments), kappa1=kappa1, kappa2=kappa2,
        ...
        visualization_data.append({
            ...
            "uh": result["Uh"],
            #"V": result["V"],
            "cells_1": result["cells_1"],
            "cells_2": result["cells_2"],
            "experiment_id": result["Experiment"]
        })

    # Gather results and visualization data
    all_results = comm.gather(results, root=0)
    all_visualization_data = comm.gather(visualization_data, root=0)

    comm.Barrier()

    # Root process writes results and performs visualization
    if rank == 0:
        ...
        with open(csv_file, mode="w", newline="") as file:
            ...
            writer.writerow([
                ...
                "Setup Time", "Solve Time", "Writing Time", "Total Time"
            ])
    ...
    for i, process_data in enumerate(all_visualization_data):
        for data in process_data:
            visualization_start = time.time()
            visualize_results(mesh=mesh, uh=data["uh"], cells_1=data["cells_1"], cells_2=data["cells_2"],
                              save_path=save_path, experiment_id=data["experiment_id"])
            print(f"Experiment #{i+1} visualized! ({time.time() - visualization_start})")
